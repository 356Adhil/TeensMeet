...
const nodeJsByteUtils = {
    toLocalBufferType(potentialBuffer) {
        ...
        const stringTag = potentialBuffer && potentialBuffer[Symbol.toStringTag] ? 
...
const webByteUtils = {
    toLocalBufferType(potentialUint8array) {
        const stringTag = potentialUint8array && potentialUint8array[Symbol.toStringTag]
...
const hasGlobalBuffer = typeof Buffer === 'function' && Buffer.prototype && !Buffer.prototype._isBuffer;
...
class Binary extends BSONValue {
    ...
    constructor(buffer, subType) {
        ...
        this.sub_type = subType !== null && subType !== undefined ? subType : Binary.BSON_BINARY_SUBTYPE_DEFAULT;
...
function internalCalculateObjectSize(object, serializeFunctions, ignoreUndefined) {
    ...
    if (Array.isArray(object)) {
        ...
        if (object && typeof object.toBSON === 'function') {
...
function calculateElement(name, value, serializeFunctions = false, isArray = false, ignoreUndefined = false) {
    if (value !== null && value !== undefined && typeof value.toBSON === 'function') {
        value = value.toBSON();
    }
    switch (typeof value) {
        case 'string':
...
function serializeInto(buffer, object, checkKeys, startingIndex, depth, serializeFunctions, ignoreUndefined, path) {
    ...
    if (Array.isArray(object)) {
        for (let i = 0; i < object.length; i++) {
            const key = `${i}`;
            let value = object[i];
            if (typeof value && typeof value.toBSON === 'function') {
                value = value.toBSON();
            }
...
function parse(text, options) {
    const ejsonOptions = {
        useBigInt64: options && options.useBigInt64 !== undefined ? options.useBigInt64 : false,
        relaxed: options && options.relaxed !== undefined ? options.relaxed : true,
        legacy: options && options.legacy !== undefined ? options.legacy : false
    };
