...
function applyRetryableWrites(target, db) {
    if (db && db.s.options && db.s.options.retryWrites) {
...
function resolveOptions(parent, options) {
    ...
    if (session && !session.inTransaction()) {
        const readConcern = read_concern_1.ReadConcern.fromOptions(options) || parent && parent.readConcern;
        if (readConcern) {
            result.readConcern = readConcern;
        }
        const writeConcern = write_concern_1.WriteConcern.fromOptions(options);
        if (!writeConcern) {
          writeConcern = parent ? parent.writeConcern : undefined;
        }        if (writeConcern) {
    ...
    if (options && options.readPreference) {
      readPreference = read_preference_1.ReadPreference.fromOptions(options.readPreference);
    } else if (parent && parent.readPreference) {
      readPreference = parent.readPreference;
    }
        if (readPreference) {
...
class HostAddress {
    ...
    toHostPort() {
        ...
        const host = this.host !== undefined ? this.host : '';
        const port = this.port !== undefined ? this.port : 0;        
        return { host, port };
    }
}
