...
if (options && options.session) {
    ...
    if (!session || !session.inTransaction()) {
        ...
        if (!readConcern && parent) {
            readConcern = parent.readConcern;
        }
                if (readConcern) {
            result.readConcern = readConcern;
        }
        const writeConcern = write_concern_1.WriteConcern.fromOptions(options);
        if (!writeConcern && parent) {
            writeConcern = parent.writeConcern;
        }
                if (writeConcern) {
            result.writeConcern = writeConcern;
        }
    }
    const readPreference = read_preference_1.ReadPreference.fromOptions(options);
    if (!readPreference && parent) {
